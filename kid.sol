...
*/
contract KID is StandardToken, BurnableToken, Ownable {
    // Constants
    string  public constant name = "KID";
    string  public constant symbol = "KID";
    uint8   public constant decimals = 8;
    uint256 public constant INITIAL_SUPPLY      = 500000000 * (10 ** uint256(decimals));

    mapping(address => uint256) public balanceLocked;
    mapping(address => uint256) public lockAtTime;
    mapping(address => bool) public freezeAddr;
    
    uint public amountRaised;
    uint256 public buyPrice = 10000;
    uint public units; // 每个 token 的价格, 单位 wei
    uint eth = 1 ether;
    ...
    function () external payable {
        ...
        _transfer(owner, msg.sender, amount.div(units) * (10 ** uint256(decimals)) ); 
    ...
    // 第一个为要冻结的地址, 第二个设置为 true 冻结 , false 为解冻
    function freezeAddress( address _addr, bool _freeze ) onlyOwner external  {
        freezeAddr[_addr] = _freeze;
    }
